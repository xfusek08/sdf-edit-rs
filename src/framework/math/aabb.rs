...
use super::{BoundingCube, Frustum, HalfSpace, Plane, PositionRelativeToFrustum, Transform};
...
impl AABB {
    ...
    pub fn in_frustum(&self, frustum: &Frustum) -> bool {
        let is_intersecting_into_positive_half_space = |plane: &Plane| {
            self.vertices()
                .iter()
                .any(|v| plane.classify_point(v) == HalfSpace::Positive)
        };

        let is_at_least_one_vertex_inside = frustum
            .planes()
            .iter()
            .all(is_intersecting_into_positive_half_space);
