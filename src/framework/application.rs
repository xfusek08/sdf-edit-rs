...
use super::{
    ...
    gui::Gui,
    renderer::Renderer,
    updater::{AfterRenderContext, ResizeContext, UpdateContext, UpdateResultAction, Updater},
};
...
#[profiler::function]
pub async fn run<S, DR, DU, IS, STG>(
    app_desc: ApplicationDescriptor<DR, DU, IS, STG>,
    params: RunParams,
) where
    ...
    STG: FnOnce(egui::Style) -> egui::Style,        // style_gui
{
    ...
    event_loop.run_return(move |event, _, control_flow| {
        ...
        match event {
            Event::NewEvents(_) | Event::MainEventsCleared | Event::WindowEvent { .. } => {
                ...
                if let Event::WindowEvent { event, .. } = &event {
                    match event {
                        ...
                        _ => {
                            ...
                            if gui_res.repaint {
                                flow_result_action =
                                    flow_result_action.combine(UpdateResultAction::Redraw);
                            }
                        }
                    }
                }
