use super::{vertices::Vertex, Context};
use crate::{info, warn};
...
impl<I: Debug + Copy + Clone + bytemuck::Pod + bytemuck::Zeroable> Buffer<I> {
    ...
    pub fn queue_update(&mut self, gpu: &Context, new_data: &[I]) -> bool {
        ...
        if new_data.len() > self.capacity {
            ...
            info!(
                "Buffer: \"{}\": Reallocating with new capacity and data {}/{} -> {}/{}",
                self.label(),
                self.size,
                self.capacity,
                new_data.len(),
            ...
            self.buffer.slice(..).get_mapped_range_mut()[..data_slice.len()]
    ...
    pub fn resize(&mut self, gpu: &Context, new_capacity: usize) -> bool {
        ...
        if new_capacity > self.capacity {
            ...
            info!(
                "Buffer \"{}\": Reallocating with new capacity {}/{} -> {}/{} (asked for {})",
                self.label(),
                self.size,
                self.capacity,
                self.size,
...
fn init_vertex_buffer<V: Vertex>(
    label: Option<&'static str>,
    vertices: &[V],
    context: &Context,
) -> wgpu::Buffer {
...
impl VertexBuffer {
    ...
    pub fn update<V: Vertex>(&mut self, context: &Context, vertices: &[V]) {
        ...
        if vertices.len() > self.capacity {
            ...
            profiler::call!(context.queue.write_buffer(
                &self.buffer,
                0,
