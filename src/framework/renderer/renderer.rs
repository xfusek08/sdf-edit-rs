...
use crate::{
    error,
    framework::{camera::SceneWithCamera, gpu, gui::Gui},
};

use super::{camera::Camera, RenderContext, RenderModule, RenderPass};
...
impl<S: SceneWithCamera> Renderer<S> {
    ...
    pub fn register_render_pass<F>(
        &mut self,
        get_pass: F,
        modules: &[RenderModuleID],
    ) -> RenderPassID
    where
        F: FnOnce(&RenderContext) -> RenderPass,
    {
        ...
        for module_id in modules {
            if !self.modules.contains_key(*module_id) {
                panic!(
                    "Cannot register render pass: module {:?} is not registered",
                    module_id
                );
            }
        }
...
impl<S: SceneWithCamera> Renderer<S> {
    ...
    pub fn resize(&mut self, size: &winit::dpi::PhysicalSize<u32>, scale_factor: f64) {
        if size.width > 0 && size.height > 0 {
            ...
            self.context
                .gpu
                .surface
                .configure(&self.context.gpu.device, &self.context.surface_config);
    ...
    pub fn prepare(&mut self, gui: &Gui, scene: &S) {
        ...
        self.context
            .camera
            .update(&self.context.gpu.queue, scene.get_camera_rig().camera());
    ...
    pub fn render(&mut self) {
        ...
        let view = profiler::call!(output
            .texture
            .create_view(&wgpu::TextureViewDescriptor::default()));

        // Create an encoder for building a GPU commands for this frame
        let mut encoder = profiler::call!(self.context.gpu.device.create_command_encoder(
...

        {
            ...
            self.passes.values_mut().for_each(|pass| {
                ...
                pass.modules
                    ...
                    .filter_map(|m_id| {
                        let Some(module) = self.modules.get(*m_id) else {
                            error!(
                                "Render module not found: {:?} requested by pass:\n{:?}",
                                m_id, pass
                            );
                            return None;
                        };
