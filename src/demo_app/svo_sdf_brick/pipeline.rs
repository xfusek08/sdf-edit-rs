...
use crate::{
    demo_app::cube::{CubeSolidMesh, CUBE_INDICES_TRIANGLE_STRIP},
    error,
    framework::{gpu, math, renderer::RenderContext},
    info,
    sdf::{
        geometry::GeometryID,
        svo::{self, Svo},
...
use super::{BrickInstance, BrickInstances, GPUGeometryTransforms};
...
impl SvoSDFBrickPipeline {
    pub fn new(context: &RenderContext) -> Self {
        ...
        let node_pool_bind_group_layout =
            svo::NodePool::create_bind_group_layout(&context.gpu, wgpu::ShaderStages::VERTEX, true);
        ...
        let pipeline = context
            ...
            .create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                ...
                layout: Some(&context.gpu.device.create_pipeline_layout(
                ...
                vertex: wgpu::VertexState {
                    ...
                    buffers: &[
                        ...
                        wgpu::VertexBufferLayout {
                            array_stride: std::mem::size_of::<gpu::vertices::SimpleVertex>()
                        ...
                        wgpu::VertexBufferLayout {
                            ...
                            array_stride: std::mem::size_of::<BrickInstance>()
                                as wgpu::BufferAddress,
    ...
    pub fn submit_svo(
        ...
        let bottom_level = svo
            .levels
            .last()
            .expect(format!("SVO {:?} has no bottom level", id).as_str());
        ...
        let record = self
            ...
            .and_modify(|rec| {
                info!(
                    "{:?}: Modifying SVO with {} instances, nodes: {}, potential bricks: {}",
                    id,
                ...
                if rec.instance_buffer.update(gpu, instance_transforms) {
                    rec.instance_bind_group = rec
                        .instance_buffer
                        .create_bind_group(gpu, &self.instance_bind_group_layout);
                }
            })
            .or_insert_with(|| {
                info!(
                    "Adding SVO {:?} with {} instances, nodes: {}, potential bricks: {}",
                    id,
                    instance_transforms.len(),
                    node_count,
                    brick_count
                ...
                let instance_buffer =
                    GPUGeometryTransforms::from_transforms(gpu, instance_transforms);
                let instance_bind_group =
                    instance_buffer.create_bind_group(gpu, &self.instance_bind_group_layout);
                let node_pool_bind_group = svo
                    .node_pool
                    .create_bind_group(gpu, &self.node_pool_bind_group_layout);
                let brick_pool_bind_group = svo
                    .brick_pool
                    .create_read_bind_group(gpu, &self.brick_pool_bind_group_layout);
                SVORenderRecord {
                    render: true,
                    domain,
                    brick_atlas_stride,
    ...
    pub fn load_counts(&mut self, gpu: &gpu::Context) {
        let mut slices: Vec<(GeometryID, wgpu::BufferSlice)> =
            Vec::with_capacity(self.svos_to_render.len());
        {
        ...
        counters::sample!(
            "brick_selected_counter",
            counts.iter().map(|(_, cnt)| cnt).sum::<u32>() as f64
        );
    ...
    pub fn render_on_pass<'rpass>(
        ...
        pass.set_index_buffer(
            self.cube_solid_mesh.index_buffer.slice(..),
        ...
        for (id, record) in self.svos_to_render.iter() {
            ...
            info!(
                "Rendering SVO {:?} with {} instances, buffer capacity: {}",
                id, instance_count, record.brick_instance_buffer.buffer.capacity
            );
