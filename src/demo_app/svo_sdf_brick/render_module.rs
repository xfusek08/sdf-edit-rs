...
use super::{SvoBrickSelectPipeline, SvoSDFBrickPipeline};
...
impl RenderModule<Scene> for SvoSdfBricksRenderModule {
    ...
    fn prepare(&mut self, _: &Gui, scene: &Scene, context: &RenderContext) {
        self.render_pipeline
            .set_display_options(scene.display_toggles.brick_display_options);
        self.render_pipeline.set_hit_distance(scene.hit_distance);
        self.render_pipeline
            .set_max_step_count(scene.max_step_count);
        ...
        let mut buckets: Vec<Option<(GeometryID, &Geometry, Vec<Transform>)>> =
            vec![None; scene.geometry_pool.capacity()];
        {
            #[inline]
        ...
        buckets.iter().filter_map(|x| x.as_ref()).for_each(
            |(geometry_id, geometry, transforms)| {
                ...
                let Some(svo) = &geometry.svo else {
                    error!(
                        "Cannot instantiate Geometry {:?}, geometry has no SVO",
                        geometry_id
                    );
                    return;
                };
                ...
                let (gpu_transforms, brick_instances) =
                    self.render_pipeline
                        .submit_svo(&context.gpu, geometry_id, svo, &transforms);
