    ...
    fn gui_section(&mut self, _: &mut Scene, ui: &mut egui::Ui) {
        counters::with_counters!(|counters| {
            if let Some(fc) = counters.get("frame_counter") {
                ...
                ui.label(format!(
                    "Last Frame time: {:.3} ms",
                    fc.duration_of_last_sample().as_secs_f64() * 1000.0
                ));
                ui.label(format!(
                    "Frame Time Average: {:.3} ms",
                    fc.average_duration_past(100).as_secs_f64() * 1000.0
                ));
            }
            ui.separator();
            if let Some(fc) = counters.get("update_counter") {
                ...
                ui.label(format!(
                    "Last Update time: {:.3} ms",
                    fc.duration_of_last_sample().as_secs_f64() * 1000.0
                ));
                ui.label(format!(
                    "Update Time Average: {:.3} ms",
                    fc.average_duration_past(100).as_secs_f64() * 1000.0
                ));
            }
            ui.separator();
