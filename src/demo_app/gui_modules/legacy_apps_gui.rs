use crate::{
    ...
    framework::{camera::Camera, gui::GuiModule},
...
impl GuiModule<Scene> for LegacyAppsGui {
    ...
    fn gui_section(&mut self, scene: &mut Scene, ui: &mut egui::Ui) {
        egui::Grid::new("grid_1").num_columns(2).show(ui, |ui| {
            ...
            ui.add(
                egui::Slider::new(
                    &mut scene.tmp_evaluator_config.min_voxel_size,
                    Geometry::VOXEL_SIZE_RANGE,
                )
                .step_by(0.001)
                .clamp_to_range(true),
            );
        ...
        egui::Grid::new("grid_2").num_columns(2).show(ui, |ui| {
            ...
            ui.add(
                egui::Slider::new(&mut break_size, 0.01..=3.0)
                    .step_by(0.01)
                    .clamp_to_range(true),
            );
            ...
            ui.add(
                egui::Slider::new(&mut scene.hit_distance, 0.00001..=0.1)
                    .step_by(0.00001)
                    .clamp_to_range(true),
            );
            ui.end_row();
