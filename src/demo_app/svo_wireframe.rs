...
impl RenderModule<Scene> for SvoWireframeRenderModule {
    ...
    fn prepare(&mut self, _: &Gui, scene: &Scene, context: &RenderContext) {
        ...
        let values: Vec<(u32, &svo::Svo)> = scene
            .geometry_pool
            .iter()
            .filter_map(|(_, geometry)| {
                ...
                let Some(cnt) = &svo.node_pool.count() else {
                    return None;
                };
                Some((cnt.clone(), svo))
            })
        ...
        let mut encoder =
            context
                ...
                .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                    label: Some(
                        "SVO Render Module Encoder For copying vertices into node vertex buffer",
                    ),
                });
        ...
        self.pipeline
            .instance_buffer
            .resize(&context.gpu, total_count);
        ...
        {
            profiler::scope!(
                "Pushing all vertices from SVO to svo wireframe renderer vertex buffer"
            );
