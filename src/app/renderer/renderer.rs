...
use super::{
    render_pass::RenderPassAttachment,
...
pub struct Renderer {
    ...
    modules: SlotMap<RenderModuleID, Box<dyn RenderModule>>,
...
impl Renderer {
    ...
    #[profiler::function]
    pub fn prepare(&mut self, state: &State) {
        
        // Update shared GPU resource outside of individual render module scopes
        
        // update camera
        // TODO: Only if camera is dirty?
        self.context.camera.update(&self.context.gpu.queue, &state.scene.camera);
        
        // Prepare all modules
        // TODO: parallelize this
        for module in self.modules.values_mut() {
            module.prepare(state, &self.context);
        }
    }
