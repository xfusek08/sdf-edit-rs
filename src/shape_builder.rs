...
use crate::{
    ...
    sdf::{
        ...
        geometry::{GeometryOperation, GeometryEditList, GeometryEdit},
    }
};
...
impl Shape {
    ...
    pub fn build(&self) -> GeometryEditList {
        let mut result: GeometryEditList = vec![];
...
impl Shape {
    ...
    fn generate_flat_edits_recursive(shape: &Shape, target_list: &mut GeometryEditList, transform: Transform, operation: GeometryOperation, blending: f32) {
        match shape {
            Shape::Primitive(primitive) => {
    ...
    fn add_primitive_to_list(primitive: &Primitive, target_list: &mut GeometryEditList, transform: Transform, operation: GeometryOperation, blending: f32) {
        target_list.push(GeometryEdit { primitive: primitive.clone(), operation, transform, blending });
    }
    
    fn add_children_to_list(children: &Vec<ShapeRecord>, target_list: &mut GeometryEditList, transform: Transform, operation: GeometryOperation, blending: f32) {
